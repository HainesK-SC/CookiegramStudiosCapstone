...
class PromotionRepositoryTest {
	...
	void findByPromoType_returnsList() {
		...
		assertThat(percentagePromos).allMatch(p -> p.getPromoType() == PromotionTypes.PERCENTAGE);
		assertThat(percentagePromos).extracting(Promotion::getPromoCode).containsExactlyInAnyOrder("SAVE10", "SAVE15");
	}
	...
	void findByPromoValue_returnsList() {
		...
		assertThat(promos).extracting(Promotion::getPromoCode).containsExactlyInAnyOrder("FIVEBUCKS", "FIVEOFF2");
	}
	...
	void findByStartDate_returnsList() {
		...
		Promotion p1 = buildPromotion("SAVE10", "Ten percent off", PromotionTypes.PERCENTAGE, 10.0, start,
				LocalDate.of(2026, 12, 31), true);

		Promotion p2 = buildPromotion("NEWYEAR5", "Five dollars off", PromotionTypes.FIXED, 5.0, start,
				LocalDate.of(2026, 1, 31), true);

		Promotion p3 = buildPromotion("FEBSALE", "Feb promo", PromotionTypes.FIXED, 3.0, LocalDate.of(2026, 2, 1),
				LocalDate.of(2026, 2, 28), true);
		...
		assertThat(results).extracting(Promotion::getPromoCode).containsExactlyInAnyOrder("SAVE10", "NEWYEAR5");
	}
	...
	void findByEndDate_returnsList() {
		...
		Promotion p3 = buildPromotion("SPRING", "Spring promo", PromotionTypes.FIXED, 7.0, LocalDate.of(2026, 4, 1),
		...
		// Assert
		assertThat(results).hasSize(2);
		assertThat(results).extracting(Promotion::getPromoCode).containsExactlyInAnyOrder("SAVE10", "SAVE15");
	}
	...
	private Promotion buildPromotion(String promoCode, String description, PromotionTypes promoType, double promoValue,
